"""
StepFun provider adapter implementation.

Translates between the Universal AI Provider SDK's unified models
and the StepFun API format (OpenAI-compatible chat completions).
"""
...
class StepFunAdapter(BaseProviderAdapter):
    ...
    StepFun chat completions schema and normalizes responses back into
    ...
    def _format_messages(messages: list[Any]) -> list[dict[str, Any]]:
        """Convert unified ``ChatMessage`` objects into StepFun wire format."""
        formatted: list[dict[str, Any]] = []
        for msg in messages:
            msg_dict: dict[str, Any] = {"role": msg.role.value}
            if msg.content is not None:
                msg_dict["content"] = StepFunAdapter._format_content(msg.content)
            if msg.name:
    ...
    def parse_response(self, response: dict[str, Any], request: UnifiedRequest) -> UnifiedResponse:
        ...
        if not choices:
            raise ResponseParsingError("StepFun response contains no 'choices'", provider="stepfun")
    ...
    def _parse_structured(content: str, schema: type[BaseModel]) -> BaseModel:
        try:
            ...
            raise ResponseParsingError(
                "Structured output could not be parsed as JSON", provider="stepfun"
            ) from exc
        ...
        try:
            ...
            raise ResponseParsingError(
                f"Structured output validation failed: {exc}", provider="stepfun"
            ) from exc

    def handle_streaming(self, response: Any, request: UnifiedRequest) -> Iterator[StreamChunk]:
        """Parse StepFun SSE streaming response into ``StreamChunk`` objects.

        StepFun streams OpenAI-compatible ``data:`` lines.  Yields a
        ...
        try:
            ...
            raise UAINetworkError(f"StepFun streaming failed: {e}") from e
    ...
    def translate_error(self, status_code: int, error_body: Any) -> Exception:
        ...
        if status_code == 401:
            ...
            return UAIRateLimitError(f"StepFun rate limited: {error_body}", retry_after=5.0)
    ...
    def capabilities(self) -> dict[str, bool]:
        """Return StepFun's capability matrix (text + vision models)."""
        return {
            "chat": True,
            "streaming": True,
            "tools": True,
