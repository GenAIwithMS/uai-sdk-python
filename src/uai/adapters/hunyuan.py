"""
Hunyuan (Tencent Cloud) provider adapter implementation.

Translates between the Universal AI Provider SDK's unified models
and the Hunyuan API format (OpenAI-compatible chat completions).
"""
...
class HunyuanAdapter(BaseProviderAdapter):
    """Adapter for the Hunyuan (Tencent Cloud) API.

    Translates the SDK's ``UnifiedRequest`` into the OpenAI-compatible
    Hunyuan chat completions schema and normalizes responses back into
    ...
    def authenticate(self, credentials: dict[str, Any]) -> None:
        """Set up authentication with the Hunyuan API."""
        ...
        if not self._api_key:
            raise UAIAuthenticationError("Hunyuan API key required")

    def format_request(self, request: UnifiedRequest) -> dict[str, Any]:
        """Translate UnifiedRequest to Hunyuan API format."""
        body: dict[str, Any] = {}
    ...
    def _format_messages(messages: list[Any]) -> list[dict[str, Any]]:
        """Convert unified ``ChatMessage`` objects into Hunyuan wire format."""
        formatted: list[dict[str, Any]] = []
        for msg in messages:
            msg_dict: dict[str, Any] = {"role": msg.role.value}
            if msg.content is not None:
                msg_dict["content"] = HunyuanAdapter._format_content(msg.content)
            if msg.name:
    ...
    def parse_response(self, response: dict[str, Any], request: UnifiedRequest) -> UnifiedResponse:
        """Translate Hunyuan API response to UnifiedResponse.

        :raises ResponseParsingError: If the response has no usable choices.
        """
        choices = response.get("choices") or []
        if not choices:
            raise ResponseParsingError("Hunyuan response contains no 'choices'", provider="hunyuan")
    ...
    def _parse_structured(content: str, schema: type[BaseModel]) -> BaseModel:
        try:
            ...
            raise ResponseParsingError(
                "Structured output could not be parsed as JSON", provider="hunyuan"
            ) from exc
        ...
        try:
            ...
            raise ResponseParsingError(
                f"Structured output validation failed: {exc}", provider="hunyuan"
            ) from exc

    def handle_streaming(self, response: Any, request: UnifiedRequest) -> Iterator[StreamChunk]:
        """Parse Hunyuan SSE streaming response into ``StreamChunk`` objects.

        Hunyuan streams OpenAI-compatible ``data:`` lines.  Yields a
        ...
        try:
            ...
            raise UAINetworkError(f"Hunyuan streaming failed: {e}") from e

    def translate_error(self, status_code: int, error_body: Any) -> Exception:
        """Translate Hunyuan API errors to SDK exceptions."""
        if status_code == 401:
            return UAIAuthenticationError(f"Hunyuan authentication failed: {error_body}")
        elif status_code == 429:
            return UAIRateLimitError(f"Hunyuan rate limited: {error_body}", retry_after=5.0)
        elif status_code >= 500:
            return UAINetworkError(f"Hunyuan server error ({status_code}): {error_body}")
        return UAIError(f"Hunyuan API error ({status_code}): {error_body}")

    def capabilities(self) -> dict[str, bool]:
        """Return Hunyuan's capability matrix (text + vision models)."""
        return {
            "chat": True,
            "streaming": True,
            "tools": True,
