"""
Kimi (Moonshot AI) provider adapter implementation.

Translates between the Universal AI Provider SDK's unified models
and the Kimi/Moonshot API format (OpenAI-compatible chat completions).
"""
...
class KimiAdapter(BaseProviderAdapter):
    ...
    Kimi chat completions schema and normalizes responses back into
    ...
    def authenticate(self, credentials: dict[str, Any]) -> None:
        """Set up authentication with the Kimi API."""
        self._api_key = credentials.get("api_key") or credentials.get("bearer_token")
        if not self._api_key:
            raise UAIAuthenticationError("Kimi API key required")
    ...
    def _format_messages(messages: list[Any]) -> list[dict[str, Any]]:
        """Convert unified ``ChatMessage`` objects into Kimi wire format."""
    ...
    def parse_response(self, response: dict[str, Any], request: UnifiedRequest) -> UnifiedResponse:
        """Translate Kimi API response to UnifiedResponse.

        :raises ResponseParsingError: If the response has no usable choices.
        """
        choices = response.get("choices") or []
        if not choices:
            raise ResponseParsingError("Kimi response contains no 'choices'", provider="kimi")
    ...
    def handle_streaming(self, response: Any, request: UnifiedRequest) -> Iterator[StreamChunk]:
        ...
        Kimi streams OpenAI-compatible ``data:`` lines.  Yields a chunk
        ...
        try:
            ...
            raise UAINetworkError(f"Kimi streaming failed: {e}") from e

    def translate_error(self, status_code: int, error_body: Any) -> Exception:
        """Translate Kimi API errors to SDK exceptions."""
        if status_code == 401:
            return UAIAuthenticationError(f"Kimi authentication failed: {error_body}")
        elif status_code == 429:
            return UAIRateLimitError(f"Kimi rate limited: {error_body}", retry_after=5.0)
        elif status_code >= 500:
            return UAINetworkError(f"Kimi server error ({status_code}): {error_body}")
        return UAIError(f"Kimi API error ({status_code}): {error_body}")

    def capabilities(self) -> dict[str, bool]:
        """Return Kimi's capability matrix."""
        return {
            "chat": True,
            "streaming": True,
            "tools": True,
