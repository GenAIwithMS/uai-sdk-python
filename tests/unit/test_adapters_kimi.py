"""Unit tests for the Kimi (Moonshot AI) provider adapter.

These tests verify that the ``KimiAdapter`` correctly translates
a ``UnifiedRequest`` into the Kimi API JSON schema, parses responses
...
from uai.adapters import KimiAdapter
...
class TestAuthenticate:
    ...
    def test_raises_without_credentials(self):
        with pytest.raises(UAIAuthenticationError):
            KimiAdapter().authenticate({})


class TestCapabilities:
    def test_basic_capabilities(self):
        caps = KimiAdapter().capabilities()
...
class TestFormatRequest:
    def test_basic_format(self):
        body = KimiAdapter().format_request(make_request())
    ...
    def test_tools_and_choice(self):
        ...
        body = KimiAdapter().format_request(request)
...
class TestParseResponse:
    def test_parse_simple_response(self):
        ...
        result = KimiAdapter().parse_response(raw, make_request())
    ...
    def test_no_choices_raises(self):
        with pytest.raises(ResponseParsingError):
            KimiAdapter().parse_response({"choices": []}, make_request())
    ...
    def test_structured_output_parsed(self):
        ...
        result = KimiAdapter().parse_response(raw, make_request(output_schema=Summary))
...
class TestTranslateError:
    def test_401(self):
        err = KimiAdapter().translate_error(401, "bad key")
        assert isinstance(err, UAIAuthenticationError)

    def test_429_includes_retry(self):
        err = KimiAdapter().translate_error(429, "slow down")
