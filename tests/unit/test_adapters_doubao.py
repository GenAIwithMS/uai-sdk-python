"""Unit tests for the Doubao (ByteDance Ark) provider adapter.

These tests verify that the ``DoubaoAdapter`` correctly translates
a ``UnifiedRequest`` into the Doubao API JSON schema, parses responses
...
from uai.adapters import DoubaoAdapter
...
class TestFormatRequest:
    def test_basic_format(self):
        body = DoubaoAdapter().format_request(make_request())
    ...
    def test_tools_and_choice(self):
        ...
        body = DoubaoAdapter().format_request(request)
...
class TestParseResponse:
    def test_parse_simple_response(self):
        ...
        result = DoubaoAdapter().parse_response(raw, make_request())
    ...
    def test_no_choices_raises(self):
        with pytest.raises(ResponseParsingError):
            DoubaoAdapter().parse_response({"choices": []}, make_request())
    ...
    def test_structured_output_parsed(self):
        ...
        result = DoubaoAdapter().parse_response(raw, make_request(output_schema=Summary))
...
class TestTranslateError:
    def test_401(self):
        err = DoubaoAdapter().translate_error(401, "bad key")
        assert isinstance(err, UAIAuthenticationError)

    def test_429_includes_retry(self):
        err = DoubaoAdapter().translate_error(429, "slow down")
...
class TestHandleStreaming:
    ...
    def test_yields_content_chunks(self):
        ...
        assert chunks[0].provider == "doubao"
